...
public class BaseHandler<T extends BaseHandler<T>> {
  ...
  public T enterGauntlet(int model) {
    try {
      ...
      while (true) {
        ...
        ADBUtil.screenShot();
        ADBUtil.copyScreenShot(Config.screenShotSavePath1[0]);
        Future<Map<String, Integer>> future1, future2;
        future1 = ThreadUtil.execAsync(() -> getCoordinate(TempImgConfig.GAUNTLET[0]));
        future2 = ThreadUtil.execAsync(() -> getCoordinate(TempImgConfig.RANKS[0], Config.screenShotSavePath1[0]));
        result1 = future1.get();
        if (model != 1) {
          break;
        }
  ...
  public T clickReward(int model) {
    try {
      ...
      while (true) {
        ...
        future2 = ThreadUtil.execAsync(() -> getCoordinate(TempImgConfig.REWARD[1], Config.screenShotSavePath1[0]));
        reward1 = future1.get();
  ...
  public void click(String tempImg) {
    ...
    // 匹配坐标
    var result = ImgMatch.match(tempImg, Config.screenShotSavePath[0]);
    try {
      // 点击该坐标
  ...
  public Map<String, Integer> getCoordinate(String tempImg) {
    // 匹配坐标并返回
    return ImgMatch.match(tempImg, Config.screenShotSavePath[0]);
  }

  public Map<String, Integer> getCoordinate(String tempImg, String targetImg) {
    // 匹配坐标并返回
    return ImgMatch.match(tempImg, targetImg);
  }
